...
__global__
void gaussian_filter(unsigned char *newImagetmp, unsigned char *in_pixelstmp, int width, int height, size_t pitch)
{
	// create kernel
...
void edge_detector(unsigned char** h_newImg, unsigned char** h_imgbuff, const int WIDTH, const int HEIGHT) {
	...
	size_t pitch1;
	cudaError_t err = cudaMallocPitch(&d_imgbuff, &pitch1, WIDTH, HEIGHT);
	if (err == 0)    cout << "cuda2D d_imgbuff finish" << endl;
	else cout << "Error :" << err << endl;
	size_t pitch2;
	err = cudaMallocPitch(&d_newImage, &pitch2, WIDTH, HEIGHT);
